...
import libraries.jsetl.exception.NotPFunException;
...
public class LMap extends LRel implements Cloneable, Visitable{
		...
		for (int i = 0; i < elements.size(); i++) {
			...
			if (elements.subList(i, elements.size()).stream().anyMatch(remainingElement -> {
				...
				if (p1.isInitialized() && p2.isInitialized()) {
					...
					if (equals(x1, x2) && isGround(y1) && isGround(y2) && !equals(y1, y2))
						return true;
				}
	 ...
	 * */
	public @NotNull Constraint pfun(){
		return new Constraint(Environment.pfunCode, this, null);
	}
    ...
    public @NotNull Constraint dom(@NotNull LSet dom){
    	...
    	return new Constraint(Environment.pfDomCode, this, dom).and(pfun());
    }
    ...
    public @NotNull Constraint ran(@NotNull LSet ran){
    	...
    	return new Constraint(Environment.pfRanCode, this, ran).and(pfun());
    }
	 ...
	 * @return the constructed constraint.
	 */
	public @NotNull Constraint comp(@NotNull LMap lMap1, @NotNull LMap lMap2){
		Objects.requireNonNull(lMap1);
		Objects.requireNonNull(lMap2);

		return new Constraint(Environment.pfCompCode, this, lMap1, lMap2).and(pfun()).and(lMap1.pfun()).and(lMap2.pfun());
	}
