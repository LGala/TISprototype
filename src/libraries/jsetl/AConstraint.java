 ...
 * Examples include x = y + 4, libraries.z != j, constraint_1 or constraint_2, ...
...
class AConstraint implements Visitable {
     ...
     * The arguments are copied by reference if they are not {@code Constraint} or {@code AConstraint},
    	...
    	for (int i=1; i <= 4; i++) {
    		...
    		if (argument != null)
    		    if(argument instanceof AConstraint && ! ((AConstraint) argument).isGround())
    		        return false;
    		    else if(argument instanceof Constraint && ! ((Constraint) argument).isGround())
    		        return false;
     ...
     * @return a {@code Constraint} which stands for the conjunction of {@code this} atomic constraint
     * with the atomic constraint {@code aConstraint}.
     */
    protected @NotNull Constraint and(@NotNull AConstraint aConstraint) {
        assert aConstraint != null;
     ...
     */
    protected @NotNull Constraint and(@NotNull Constraint constraint) {
     ...
     */
    protected @NotNull AConstraint or(@NotNull Constraint constraint) {
        assert constraint != null;
     ...
     * Only arguments of type {@code AConstraint} or {@code Constraint}
    ...
    private @Nullable Object cloneArgumentIfNeeded(@Nullable Object argument){
        ...
        else if(argument instanceof Constraint)
            returnedObject = ((Constraint) argument).clone();
        else
