...
class ConstraintMapper {
     ...
     */
    @NotNull
    protected Constraint mapConstraintDeeply(@NotNull final Constraint constraint){
        assert constraint != null;

        final Constraint mappedConstraint = mapDeeply(constraint, original, mapped);
     ...
     */
    @NotNull
    private Constraint mapDeeply(@NotNull final Constraint constraint, @NotNull Object original, @NotNull Object mapped){
        assert constraint != null;
        assert original != null;
        assert mapped != null;

        Constraint replaced = new Constraint();
     ...
     */
    @NotNull
    private Constraint mapDeeply(@NotNull AConstraint aConstraint, @NotNull Object original, @NotNull Object mapped){
        assert aConstraint != null;
        assert original != null;
        assert mapped != null;

        if(aConstraint.constraintKindCode == Environment.orCode){
            Constraint orAConstraintMapped = mapOrAConstraintDeeply(aConstraint, original, mapped);
        ...
        return new Constraint(mappedAConstraint);
    }
     ...
     */
    @NotNull
    private Constraint mapOrAConstraintDeeply(@NotNull AConstraint aConstraint, @NotNull Object original, @NotNull Object mapped) {
        assert aConstraint != null;
        assert original != null;
        assert mapped != null;
        ...
        Constraint firstCase = new Constraint((Constraint) aConstraint.argument1);
        Constraint secondCase = new Constraint((Constraint) aConstraint.argument2);

        Constraint firstReplaced = mapDeeply(firstCase, original, mapped);
        Constraint secondReplaced = mapDeeply(secondCase, original, mapped);

        Constraint mappedConstraint = firstReplaced.or(secondReplaced);
