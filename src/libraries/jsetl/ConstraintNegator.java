...
import static libraries.jsetl.Environment.*;
import static libraries.jsetl.Environment.trueCode;
...
class ConstraintNegator{
     ...
     */
    protected @NotNull Constraint negate(@NotNull Constraint constraint){
        ...
        if(constraint.size() == 1)
            if(!constraint.get(0).getDefinitionConstraint())
                ...
                return new Constraint(constraint.get(0).clone());

        Constraint result = negateAnd(constraint);
        assert result != null;
        return result;
    }
     ...
     */
    private @NotNull Constraint negate(@NotNull AConstraint aConstraint){
        ...
        if (aConstraint.getDefinitionConstraint() || aConstraint.constraintKindCode == labelCode)
            return new Constraint(aConstraint.clone());
     ...
     * constraints of unsupported kinds.
     */
    private @NotNull Constraint negateOr(@NotNull AConstraint or){
        assert or != null;
        assert or.constraintKindCode == orCode;
        assert or.argument1 != null;
        assert or.argument1 instanceof AConstraint || or.argument2 instanceof Constraint;

        assert or.argument2 != null;
        assert or.argument2 instanceof Constraint || or.argument2 instanceof Constraint;
        ...
        if(or.argument1 instanceof AConstraint)
            first = new Constraint((AConstraint) or.argument1);
        else if(or.argument1 instanceof Constraint)
            first =(Constraint) or.argument1;

        Constraint second = null;
        if(or.argument2 instanceof AConstraint)
            second = new Constraint((AConstraint) or.argument2);
        else if(or.argument2 instanceof Constraint)
            second = (Constraint) or.argument2;

        Constraint firstNegated = negate(first);
        Constraint secondNegated = negate(second);

        Constraint result = firstNegated.and(secondNegated);
        assert result != null;
        return result;
    }
     ...
     */
    private @NotNull Constraint negateAnd(@NotNull Constraint and){
        assert and != null;
        assert and.size() >= 2;
        ...
        Constraint firstNegated = negate(firstConstraint);
        Constraint othersNegated = negate(aConstraints.get(aConstraints.size() - 1));
        ...
        if(and.get(and.size() - 2).getDefinitionConstraint() && and.get(and.size() - 2).constraintKindCode == Environment.eqCode || and.get(and.size() - 1).getDefinitionConstraint())
            constraintType = "_and";
        return new Constraint(constraintType, firstNegated, othersNegated);
    }
     ...
     */
    private @NotNull Constraint negateEq(@NotNull AConstraint eq){
        assert eq != null;
        ...
        return new Constraint(new AConstraint(neqCode, eq.argument1, eq.argument2));
    }
     ...
     */
    private @NotNull Constraint negateNeq(@NotNull AConstraint neq){
        assert neq != null;
        ...
        return new Constraint(new AConstraint(eqCode, neq.argument1, neq.argument2));
    }
     ...
     */
    private @NotNull Constraint negateIn(@NotNull AConstraint in){
        assert in != null;
        ...
        return new Constraint(new AConstraint(ninCode, in.argument1, in.argument2));
    }
     ...
     */
    private @NotNull Constraint negateNin(@NotNull AConstraint nin){
        assert nin != null;
        ...
        return new Constraint(new AConstraint(inCode, nin.argument1, nin.argument2));
    }
     ...
     */
    private @NotNull Constraint negateSubset(@NotNull AConstraint subset){
        assert subset != null;
        ...
        return new Constraint(new AConstraint(nsubsetCode, subset.argument1, subset.argument2));
    }
     ...
     */
    private @NotNull Constraint negateLt(@NotNull AConstraint lt){
        assert lt != null;
        ...
        return new Constraint(new AConstraint(Environment.geCode, lt.argument1, lt.argument2));
    }
     ...
     */
    private @NotNull Constraint negateLe(@NotNull AConstraint le){
        assert le != null;
        ...
        return new Constraint(new AConstraint(Environment.gtCode, le.argument1, le.argument2));
    }
     ...
     */
    private @NotNull Constraint negateGt(@NotNull AConstraint gt){
        assert gt != null;
        ...
        return new Constraint(new AConstraint(Environment.leCode, gt.argument1, gt.argument2));
    }
     ...
     */
    private @NotNull Constraint negateGe(@NotNull AConstraint ge){
        assert ge != null;
        ...
        return new Constraint(new AConstraint(Environment.ltCode, ge.argument1, ge.argument2));
    }
     ...
     */
    private @NotNull Constraint negateSize(@NotNull AConstraint size){
        assert size != null;
        ...
        Constraint trueSizeConstraint = new Constraint("_size", size.argument1, trueSize);
     ...
     */
    private @NotNull Constraint negateTrue(@NotNull AConstraint truec){
        assert truec != null;
        assert truec.constraintKindCode == trueCode;

        return new Constraint(new AConstraint(falseCode, null, null));
    }
     ...
     */
    private Constraint negateFalse(AConstraint falsec){
        assert falsec != null;
        assert falsec.constraintKindCode == falseCode;

        return new Constraint(new AConstraint(trueCode, null, null));
    }
