...
class RwRulesSet extends LibConstraintsRules {
     ...
     */
    public RwRulesSet(@NotNull Solver solver) {
        super(solver);
        assert solver != null;
    ...
    protected void in(@NotNull AConstraint aConstraint) {
        ...
        if(aConstraint.argument2 instanceof LVar && ((LVar) aConstraint.argument2).getValue() instanceof LSet) {
            aConstraint.argument2 = ((LSet)((LVar) aConstraint.argument2).getValue()).getEndOfEquChain();
        }
        if (aConstraint.argument2 instanceof SetLVar)                      // intlvar.in(setlvar)
            ...
            assert(false); //the control should not reach this point.
    }
    ...
    private void inLVarCP(@NotNull LVar lVar, @NotNull CP cp, @NotNull AConstraint aConstraint) {
        ...
        aConstraint.constraintKindCode = Environment.eqCode;            // libraries.z = (n1,n2)
    ...
    private void inRis(@NotNull Object object, @NotNull Ris ris, @NotNull AConstraint aConstraint) {
        ...
        if(!rp.isEmpty())
            solver.add(rp);
        AConstraint aConstraint1 = new AConstraint(Environment.eqCode, object, p);
        solver.add(aConstraint1);
        solver.storeUnchanged = false;
        eqHandler.eq(aConstraint1);
        in(aConstraint);
        ...
        if (lSet.isBound()) { // (1)
            if (lSet.isEmpty()) {
                ...
                if(solver.getOptimizationOptions().areInOptimizationsEnabled()) {
                    boolean allground = LObject.isGround(lVar);
                    for (Object o : lSet) {
                        if(!LObject.isGround(o))
                            allground = false;
                        if (LObject.equals(o,lVar)) {
    ...
    private void inObjSet(@NotNull Object object, @NotNull LSet lSet,@NotNull AConstraint aConstraint) {
    	...
    	if (lSet.getTail() instanceof CP) {
            ...
            if (lSet.isEmpty()) {
                ...
                if(solver.getOptimizationOptions().areInOptimizationsEnabled()) {
                    boolean allground = LObject.isGround(object);
                    for (Object o : lSet) {
                        if(!LObject.isGround(o))
                            allground = false;
                        if (LObject.equals(o,object)) {
                            aConstraint.setSolved(true);
                            return;
                        }
                    }
    ...
    private void disj(@NotNull LSet lSet1, @NotNull LSet lSet2, @NotNull AConstraint aConstraint) {
        ...
        if (!lSet1.isBound() && !lSet2.isBound()) { // x disj y  (solved)
            ...
            Constraint st = new Constraint(Environment.ninCode, lSet1.getOne(),
                    lSet2); // t1.nin(X)
            solver.add(st);
            solver.storeUnchanged = false;
        } else if (!lSet1.isBound() && lSet2.isBound()) { // X || {t2|s2} //                                                        // (3)
            aConstraint.argument2 = lSet2.removeOne(); // X||s2
            Constraint se = new Constraint(Environment.ninCode, lSet2.getOne(),
                    lSet1); // t2.nin(X)
            solver.add(se);
            solver.storeUnchanged = false;
        } else { // {t1|s1}|| {t2|s2} (4)
            ...
            Constraint s1 = new Constraint(Environment.neqCode, lSet1.getOne(),
                    lSet2.getOne()); // t1.neq(t2)
            Constraint s2 = new Constraint(Environment.ninCode, lSet1.getOne(),
                    lSet2.removeOne()); // t1.nin(s2)
            Constraint s3 = new Constraint(Environment.ninCode, lSet2.getOne(),
    ...
    private void unionCP(@NotNull LSet lSet1, @NotNull LSet lSet2, @NotNull LSet lSet3, @NotNull AConstraint aConstraint) {
        ...
        if((!lSet1.isVariable() && lSet1 instanceof CP)
            ...
            LSet N2 = new LSet();
            LSet N3 = new LSet();
            Constraint c1 = UCP(lSet1, N1);
            Constraint c2 = UCP(lSet2, N2);
            Constraint c3 = UCP(lSet3, N3);
     ...
     * @author Marco Ghezzi
     */
    private @NotNull Constraint UCP(@NotNull LSet t, @NotNull LSet Ni) {
        assert t != null;
        assert Ni != null;
     ...
     *
     */
    private @NotNull Constraint K_i(@NotNull LSet A, @NotNull LSet N){
        assert A != null;
        assert N != null;
        ...
        Object d = domain.getOne();
        Constraint filterApply = ris.F(d);
        Constraint cc = new Constraint();
        Object p = ris.P(d,cc);
        if(cc.isEmpty())
            cc = Constraint.truec();
        ...
        Constraint returnedConstraint = (N.eq(newSet).and(filterApply).and(cc)).or(N.eq(newRis).and(ris.notF(d)));
    ...
    private void detSize(@NotNull LSet lSet, @NotNull IntLVar size, @NotNull AConstraint aConstraint) {
        ...
        if (lSet.isGround()) {
            ...
            if (k > 1)
                solver.add(Constraint.allDifferent(newS.toArrayList()));
