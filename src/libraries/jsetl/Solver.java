...
import libraries.jsetl.ris.cache.RisExpansionCache;
...
import libraries.jsetl.ris.cache.CacheKey;
...
public class Solver {
    ...
    ///////////////////////////////////////////////////////////////
    //////////////// UNSAFE RIS OPERATIONS MANAGEMENT /////////////
    ///////////////////////////////////////////////////////////////

    /*
        The Strict Ris Mode is a mode in which if an unsafe operation
        on a RIS is performed, namely the negation of a filter with
        existential variables that are not functional (a functional
        variable is, at the moment, the result of an arithmetic operation),
         then an exception of type{@code Solver.UnsafeRisException} is thrown.
        When the Strict Ris Mode is turned off the same event sets a flag
        to {@code true} that can be tested afterwards.
        The Strict Ris Mode is turned ON by default.
    ...
    /**
     * This flag is set to {@code true} if the Strict Ris Mode if OFF and
     * an unsafe RIS operations is performed. {@code false} by default.
     */
    private static boolean unsafeRisUsageWarning = false;

    /**
     * This flag is used to activate or deactivate the Strict Ris Mode. {@code true} by default.
     * If Strict Ris Mode is ON and the exception is thrown the solver is left
     *         in an inconsistent state so it should not be reused, nor should the result of the
     *         operation that caused the throw of the exception be trusted.
     */
    private static boolean useStrictRisMode = true;

    /**
     * Sets the Strict Ris Mode ON ({@code true}) or OFF ({@code false}).
     * Moreover, calling this method resets to {@code false} the value
    ...
    public static void setUseStrictRisMode(boolean value){
        useStrictRisMode = value;
        unsafeRisUsageWarning = false;
    }

    /**
     * Tells whether the Strict Ris Mode is ON ({@code true}) or OFF ({@code false}).
     * The Strict Ris Mode is ON by default.
     * If Strict Ris Mode is ON and the exception is thrown the solver is left
     *         in an inconsistent state so it should not be reused, nor should the result of the
     *         operation that caused the throw of the exception be trusted.
     * @return {@code true} if the Strict Ris Mode is on, {@code false} if the Strict Ris Mode is off
     */
    public static boolean getUseStrictRisMode(){
        return useStrictRisMode;
    }

    /**
     * Tells whether the warning flag is {@code true} or not but does not change its value.
     * @return {@code true} if the warning flag is set to {@code true}, {@code false} otherwise.
     */
    public static boolean peekUnsafeRisUsageWarning(){
        return unsafeRisUsageWarning;
    }

    /**
     * Tells whether the warning flag is {@code true} or not and does changes its value to {@code false}.
     * @return {@code true} if the warning flag is set to {@code true}, {@code false} otherwise.
     */
    public static boolean getUnsafeRisUsageWarning(){
        boolean temp = unsafeRisUsageWarning;
        unsafeRisUsageWarning = false;
        return temp;
    }
     ...
     *         operation that caused the throw of the exception be trusted.
     */
    static public class UnsafeRisException extends RuntimeException{
        public UnsafeRisException(){
            super();
        }
    }

    /**
     * Method used internally by unsafe RIS operations to signal the unsafe operation.
     * Calling this method will throw an {@code UnsafeRisException} if the Strict Ris Mode
     * is ON, it will set the warning flag to {@code true} otherwise.
     * If Strict Ris Mode is ON and the exception is thrown the solver is left
     *         in an inconsistent state so it should not be reused, nor should the result of the
     *         operation that caused the throw of the exception be trusted.
     */
    static void signalUnsageRisUsageWarning(){
        if(useStrictRisMode)
            throw new UnsafeRisException();
        unsafeRisUsageWarning = true;
    }
    ...
    protected static boolean checkUnification(Object a, Object b){
        ...
        Constraint equalityConstraint = new Constraint(Environment.eqCode, a, b);
     ...
     */
    private Solver lastExecutingSolver;
     ...
     */
    public void addChoicePoint(@NotNull Constraint constraint) {
        Objects.requireNonNull(constraint);
    ...
    public @NotNull Constraint getConstraint() {
        Constraint result = this.store.getConstraint();
        assert result != null;
        return result;
    }
    ...
    */ 
    public boolean check(@NotNull Constraint constraint) {
        Objects.requireNonNull(constraint);
     ...
     */
    public void solve(@NotNull Constraint constraint) throws Failure {
        Objects.requireNonNull(constraint);
        this.add(constraint);
        this.solve();
    }
    ...
    public boolean test() {
        ...
        ChoicePoint backupChoicePoint = this.backtracking.createBackupChoicePoint();
        Constraint cCopy = this.getConstraint().clone();
    ...
    public @NotNull Integer maximize(@NotNull IntLVar intLVar) {
        ...
        Integer max = Integer.MIN_VALUE/2 + 1;
        findNotInitializedLogicalObjectsInStore();
        ChoicePoint backupChoicePoint = this.backtracking.createBackupChoicePoint();
     ...
     */
    protected void solve(Constraint cc, int lev)
    ...
    protected void risNoLabel()
            throws Fail {
        int i = 0;
        boolean storeUnchanged = true;

        do {
            this.storeUnchanged = true;

            for (int j = 0; j < store.size(); ++j) {
                AConstraint tmp = this.get(j);
                if (!tmp.getSolved() && tmp.isGround()) risAConstraint(tmp);
            }
            if (this.storeUnchanged == false)
                storeUnchanged = false;
        } while (this.storeUnchanged == false);

        this.storeUnchanged = storeUnchanged;
        while (i < store.size()) {
            AConstraint tmp = this.get(i);
            if (!tmp.getSolved() && tmp.constraintKindCode != Environment.labelCode)
                risAConstraint(tmp);
            ...
            if (aConstraint.constraintKindCode < Environment.FIRST_NEW_CONSTRAINTS_CODE) {
                ...
                if (!newConstraints.isEmpty()) {
                    ...
                    while(itr.hasNext()) {
                        CN = itr.next();
                        try {
                            CN.user_code(new Constraint(aConstraint));
                            rewritten = true;
                            break;
                        }
    ...
    /**
     * Raises an exception {@ode Fail}. This exception is handled by the {@code Solver} which
