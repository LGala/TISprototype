...
class RisExpansor {
     ...
     * @see #expand(Solver)
     */
    protected boolean canExpand(@NotNull Solver solver){
        assert solver != null;
     ...
     * @see #canExpand(Solver)
     */
    protected @NotNull
    LSet expand(@NotNull Solver solver){
        assert solver != null;
     ...
     */
    private boolean canExpand(@NotNull Solver solver, @NotNull Object element){
        assert solver != null;
        assert element != null;
     ...
     */
    boolean filterHasExistentialVariables(){
        ...
        for(LObject variable : filterVariables)
            if(isDummyVariable(variable) && !(variable instanceof IntLVar && ((IntLVar) variable).isDummy()))
                return true;
        return false;
    }
