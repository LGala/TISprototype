...
public class LRel extends LSet implements Cloneable, Visitable{
	...
	protected static boolean isDomainGround(@NotNull LSet lSet){
		...
		while(rest.getSize() >= 1){
			p = (LPair)rest.getOne();
			rest = rest.removeOne();
			if(!p.isBound() || !isGround(p.getFirst()))
				return false;
		}
		return true;
	}
     ...
     */
    public @NotNull Constraint dom(@NotNull LSet dom){
    	Objects.requireNonNull(dom);

    	return new Constraint(Environment.brDomCode, this, dom);
    }
     ...
     */
    public @NotNull Constraint ran(@NotNull LSet ran){
    	Objects.requireNonNull(ran);

    	return new Constraint(Environment.brRanCode, this, ran);
    }   
     ...
     * @return the constructed constraint.
     */
    public @NotNull Constraint comp(@NotNull LRel lRel1, @NotNull LRel lRel2){
    	Objects.requireNonNull(lRel1);
    	Objects.requireNonNull(lRel2);

    	return new Constraint(Environment.brCompCode, this, lRel1, lRel2);
    }
	 ...
	 * @return the constructed constraint.
	 */
	public @NotNull Constraint id(@NotNull LSet lSet){
	 	Objects.requireNonNull(lSet);

		return new Constraint(Environment.idCode, this, lSet);
	}
	 ...
	 * @return the constructed constraint.
	 */
	public @NotNull Constraint inv(@NotNull LRel lRel){
		Objects.requireNonNull(lRel);

		return new Constraint(Environment.invCode, this, lRel);
	}
	 ...
	 * @return the constructed constraint.
	 */
	public @NotNull Constraint id(){
		LSet a = new LSet();

		Constraint domr = this.dom(a);
		Constraint ranr = this.ran(a);
		Constraint compr = this.comp(this, this);
		Constraint returned = domr.add(ranr).add(compr);
	 ...
	 * @return the constructed constraint.
	 */
	public @NotNull Constraint dres(@NotNull LSet lSet, @NotNull LRel lRel){
		Objects.requireNonNull(lRel);
		Objects.requireNonNull(lSet);
		...
		Constraint dom1 = lRel.dom(N2);
		Constraint dom2 = N1.dom(N3);
		Constraint un = lRel.union(N1, this);
		Constraint sub = N2.subset(lSet);
		Constraint disj = lSet.disj(N3);

		Constraint returned = dom1.and(dom2).and(un).and(sub).and(disj);
	 ...
	 * @return the constructed constraint.
	 */
	public @NotNull Constraint rres(@NotNull LSet lSet, @NotNull LRel lRel){
		Objects.requireNonNull(lRel);
		Objects.requireNonNull(lSet);
		...
		Constraint ran1 = lRel.ran(N2);
		Constraint ran2 = N1.ran(N3);
		Constraint un = this.union(N1, lRel);
		Constraint sub = N2.subset(lSet);
		Constraint disj = lSet.disj(N3);

		Constraint returned = ran1.and(ran2).and(un).and(sub).and(disj);
	...
	* @return the constructed constraint.
	*/
	public @NotNull Constraint ndom(@NotNull LSet lSet){
		Objects.requireNonNull(lSet);
		...
		Constraint part1 = p.in(this).and(n1.nin(lSet));
		Constraint part2 = n1.in(lSet).and(m.comp(this, empty));

		Constraint returned = part1.or(part2);
	 ...
	 * @return the constructed constraint.
	 */
	public @NotNull Constraint nran(@NotNull LSet lSet){
		Objects.requireNonNull(lSet);
		...
		Constraint part1 = p.in(this).and(n2.nin(lSet));
		Constraint part2 = n1.in(lSet).and(m.comp(this, empty));
	 ...
	 * @return the constructed constraint.
	 */
	public @NotNull Constraint ninv(@NotNull LRel lRel){
		Objects.requireNonNull(lRel);
		...
		Constraint part1 = p1.in(this).and(p2.nin(lRel));
		Constraint part2 = p1.nin(this).and(p2.in(lRel));
	 ...
	 * @return the constructed constraint.
	 */
	public @NotNull Constraint nid(@NotNull LSet lSet){
		Objects.requireNonNull(lSet);
		...
		Constraint part1 = n1.in(lSet).and(p1.nin(this));
		Constraint part2 = n1.nin(lSet).and(p1.in(this));
		Constraint part3 = n1.neq(n2).and(p2.in(this));

		Constraint returned = part1.or(part2).or(part3);
	 ...
	 * @return the constructed constraint.
	 */
	public @NotNull Constraint ncomp(@NotNull LRel lRel1, @NotNull LRel lRel2){
		Objects.requireNonNull(lRel1);
		Objects.requireNonNull(lRel2);
		...
		Constraint part2 = p3.in(lRel2).and(m1.comp(this, N1)).
	 ...
	 * @return the constructed constraint.
	 */
	public @NotNull Constraint ndres(@NotNull LSet lSet, @NotNull LRel lRel){
		Objects.requireNonNull(lRel);
		Objects.requireNonNull(lSet);

		LRel b = new LRel();

		Constraint dre = dres(lSet, b);
		Constraint dif = this.diff(b, lRel);
		Constraint un = lSet.union(b, lRel);

		Constraint returned = dre.add(dif);
	 ...
	 * @return the constructed constraint.
	 */
	public @NotNull Constraint nrres(@NotNull LSet lSet, @NotNull LRel lRel){
		Objects.requireNonNull(lRel);
		Objects.requireNonNull(lSet);

		LRel a = new LRel();

		Constraint rre = this.rres(a, lRel);
		Constraint dif = this.diff(a, lRel);

		Constraint returned = rre.add(dif);
